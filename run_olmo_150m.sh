#!/bin/bash
#
# Run OLMo 150M training on 4 GPUs for a given norm type.
#
# Config selection (in priority order):
#   1. configs/tiny/OLMo-150M-local.yaml   – locally-downloaded shards (no streaming)
#   2. configs/tiny/OLMo-150M-public.yaml  – streams from https://olmo-data.org/
#
# Generate the local config with:
#   python scripts/download_olmo_data.py --data-dir /dev/shm/ssrivastava/datasets/olmo-data
...
#
# Usage:
#   ./run_olmo_150m.sh <norm_type> [master_port]
#
# Arguments:
...
if [ -z "$norm_type" ]; then
    ...
    echo "Usage: ./run_olmo_150m.sh <norm_type> [master_port]"
    exit 1
fi
...
echo "=========================================="
echo "OLMo 150M training on 4 GPUs"
echo "=========================================="
