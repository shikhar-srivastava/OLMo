#!/bin/bash
#
# Run OLMo 1B training on 4 GPUs for a given norm type.
#
# Config selection (in priority order):
#   1. configs/official-0724/OLMo-1B-local.yaml  – locally-downloaded shards (no streaming)
#   2. configs/official-0724/OLMo-1B.yaml        – streams from https://olmo-data.org/
#
...
#
# Usage:
#   ./run_olmo_1b.sh <norm_type> [master_port]
#
# Arguments:
...
#                   default – passed through directly to model.layer_norm_type
...
if [ -z "$norm_type" ]; then
    ...
    echo "Usage: ./run_olmo_1b.sh <norm_type> [master_port]"
    exit 1
fi
...
# "lns"  → LayerNorm Scaling (RMSNorm + 1/sqrt(layer_id+1) depth scaling)
# anything else is forwarded verbatim (e.g. "rms", "default", "low_precision")
case "$norm_type" in
...
# If a master port is provided use it, otherwise fall back to 29500.
if [ -n "$2" ]; then
...
echo "=========================================="
echo "OLMo 1B training on 4 GPUs"
echo "=========================================="
