#!/bin/bash
#
# Run OLMo 60M training on 4 GPUs for a given norm type.
#
# Config selection (in priority order):
#   1. configs/tiny/OLMo-60M-local.yaml   – locally-downloaded shards (generated by
#                                            scripts/download_olmo_data.py).  No internet
#                                            traffic during training; fastest I/O.
#   2. configs/tiny/OLMo-60M-public.yaml  – streams shards from https://olmo-data.org/.
#                                            Used automatically if no local config exists.
#
...
if [ -z "$norm_type" ]; then
    ...
    echo "Usage: ./run_olmo_60m.sh <norm_type> [master_port]"
    exit 1
fi
...
# W&B: TrainConfig defaults wandb.entity to "ai2-llm" (Ai2). Without overriding, runs go to
# ai2-llm/<project> and fail with 403 for non-Ai2 accounts. Use null for your default login entity,
# or: export WANDB_ENTITY=your-team-slug  (e.g. klab-shikhar) before running.
...
echo "=========================================="
echo "OLMo 60M training on 4 GPUs"
echo "=========================================="
